...
BaseType_t xSwitchRequired = pdFALSE;
	...
	if( uxSchedulerSuspended == ( UBaseType_t ) pdFALSE )
	{
        ...
        uint16_t * comp = NULL;
        portGET_OS_TICK_COMPENSATE_VAL(&comp);
        const TickType_t last_tick_count = xTickCount;
		const TickType_t xConstTickCount = xTickCount + 1 + (*comp);
        *comp = 0;
        #else
        ...
        #if ( defined(OS_TICK_COMPENSATE) )
		if( xConstTickCount < last_tick_count )
        #else
