...
static char g_wifi_host_name[20]={0};
...
static err_t low_level_output(struct netif *netif, struct pbuf *pkt)
{
    ...
    wifi_if_send_ethernet_pkt(frame_buf, pkt->tot_len, 30, 5);
...
static err_t ethernetif_init(struct netif *netif, wifi_interface_enum_t nif_idx)
{
    ...
    if(nif_idx == STATION_IF){
        ...
        netif->hostname = (const char *)g_wifi_host_name;//"LN_STA_'mac[4:5]'"
...
#if LWIP_NETIF_HOSTNAME
        /* Initialize interface hostname */
        netif->hostname = (const char *)g_wifi_host_name;//"LN_AP_'mac[4:5]'"
#endif /* LWIP_NETIF_HOSTNAME */
...
#if LWIP_NETIF_HOSTNAME
        /* Initialize interface hostname */
        netif->hostname = (const char *)g_wifi_host_name;//"LN_MONITOR_'mac[4:5]'"
#endif /* LWIP_NETIF_HOSTNAME */
...
int ethernetif_set_mac_addr(wifi_interface_enum_t nif_idx, uint8_t *mac_addr)
{
    ...
    if(nif && mac_addr){
        ...
        if(nif_idx == STATION_IF){
            sprintf(g_wifi_host_name, "LN_STA_%X%X",mac_addr[4],mac_addr[5]);
        }else if(nif_idx == SOFT_AP_IF){
            sprintf(g_wifi_host_name, "LN_AP_%X%X",mac_addr[4],mac_addr[5]);
        }else if(nif_idx == MONITOR_IF){
            sprintf(g_wifi_host_name, "LN_MONITOR_%X%X",mac_addr[4],mac_addr[5]);
        }
        return 0;
    }
    return -1;
...
int ethernetif_set_state(wifi_interface_enum_t nif_idx, netif_state_enum_t up)
{
    ...
    if(nif_idx == STATION_IF){
        ...
        if(up == NETIF_UP && ndev->link != LINK_UP){
            netif_set_up(nif);
            netif_set_link_up(nif);
			ndev->link = LINK_UP;
            dhcpd_stop();
            dhcpd_start();
        }else if(up == NETIF_DOWN && ndev->link != NETIF_DOWN){
            dhcpd_stop();
            netif_set_status_callback(nif, NULL);
            netif_set_link_down(nif);
            netif_set_down(nif);
